...
class AudioPre:
    ...
    def run(self, path, sound_name, root1, root2, suffix1, suffix2, format):
        assert format in ['wav', 'flac']
        if not os.path.isdir(root1): os.mkdir(root1)
        if not os.path.isdir(root2): os.mkdir(root2)
        ...
        logger.info("%s instruments done" % sound_name)

        sf.write(
            os.path.join(root1, f'{sound_name}_{suffix1}.{format}'),
        ...
        logger.info("%s vocals done" % sound_name)
        sf.write(
            os.path.join(root2, f'{sound_name}_{suffix2}.{format}'),
